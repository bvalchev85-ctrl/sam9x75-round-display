...
{
	/*
	 * Deliberately do NOT power the panel MCU down here.
	 *
	 * On this board drm_panel .enable() is only invoked once (at the
	 * initial modeset); it is not re-invoked for subsequent modesets.
	 * So once 0xAD is cleared the panel can never be switched back on
	 * without a power cycle, and every later modeset renders to a dead
	 * panel (backlight on, screen black).
	 *
	 * Leaving the MCU enabled is harmless - the DSI video stream is what
	 * actually gates the image - and it makes the panel survive
...
static int ws_panel_enable(struct drm_panel *panel)
{
	...
	/*
	 * Re-assert the FULL MCU power-on sequence (not just 0xAD) so that an
	 * enable after any disable/suspend fully re-establishes the panel
	 * rather than relying on state left over from probe().
	 */
