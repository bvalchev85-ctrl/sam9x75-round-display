...
struct ws_panel_data {
	...
	/*
	 * Physical active-area size.  Upstream hardcodes 154x86 mm for every
	 * model in ws_panel_get_modes(); that is the 7" 800x480 DPI panel and
	 * is wrong for all the others.  See the comment there for why it
...
/* 4.0inch 720x720 Round (Waveshare 4inch DSI LCD (C))
 * EXACT timing from Microchip MGS demo curiosity_ws_4in_round (plib_dsi.c /
 * plib_xlcdc.c): 50 MHz; H front/sync/back = 330/32/32 (htotal 1114);
 * V front/sync/back = 8/4/16 (vtotal 748); ~60 Hz. The large 330-cyc HFP is
 * panel-specific and required for the DSI->DPI bridge to lock.
 */
static const struct drm_display_mode ws_panel_4_mode = {
	.clock = 50000,
	.hdisplay = 720,
	.hsync_start = 720 + 330,
	.hsync_end = 720 + 330 + 32,
	.htotal = 720 + 330 + 32 + 32,
	.vdisplay = 720,
	.vsync_start = 720 + 8,
	.vsync_end = 720 + 8 + 4,
	.vtotal = 720 + 8 + 4 + 16,
...
static const struct ws_panel_data ws_panel_4_data = {
	...
	.lanes = 2,
	/*
	 * Match the MGS demo's DSI host config exactly:
	 *  - VIDEO + SYNC_PULSE  => VID_MODE_TYPE non-burst sync pulses (demo VID_MODE_TYPE(0))
	 *  - CONTINUOUS HS clock (no NON_CONTINUOUS): demo AUTO_CLKLANE_CTRL(0)
	 *  - NO_EOT_PACKET: demo sets EOTP_TX_EN(0). The dw-mipi-dsi core enables
	 *    EOTP_TX by DEFAULT; the trailing EoT packet after each video transfer
	 *    is what the panel's DSI->DPI bridge rejects => backlight on, pixels black.
	 */
...
{
	/* MGS demo sends no DCS; panel is driven by the 0x45 MCU + DSI video. */
	return 0;
}
...
static int ws_panel_enable(struct drm_panel *panel)
{
	...
	/*
	 * Brightness step, exactly as the MGS demo does it AFTER the DSI video
	 * stream is up (brightSeq): 0xAB is the panel content brightness (NOT the
	 * backlight LED). The stock RPi formula wrote 0xAB = 0xff-brightness = 0
	 * at max brightness => image invisible (backlight on, screen black).
	 */
	msleep(50);
	ws_panel_i2c_write(ts, 0xab, 55);
	ws_panel_i2c_write(ts, 0xaa, 0x01);
	dev_info(&ts->i2c->dev, "ws4round: enable done (0xAB=55)\n");
			      ...
			      struct drm_connector *connector)
{
	...
	/*
	 * Physical size MUST match the panel actually fitted.  Upstream
	 * hardcodes 154x86 mm here for every model - that is the 7" 800x480
	 * DPI panel, and it is badly wrong for this 720x720 round one (4"
	 * diagonal => a 72 x 72 mm square active area).
	 *
	 * This is not cosmetic.  Anything that corrects for the display aspect
	 * ratio believes the pixels are 154/86 = 1.79x wider than tall and
	 * squeezes every frame's width to ~3/5.  GStreamer's kmssink does
	 * exactly that: measured on this board, a *square* 720x720 source was
	 * displayed as 432x720 with black bars down both sides, and a 640x480
	 * camera feed came out 576x720 using only the left 384 columns of the
	 * source.  No pipeline setting can undo it - not render-rectangle, not
	 * display-width/height, not plane-properties - because kmssink
	...
	if (ts->data && ts->data->width_mm && ts->data->height_mm) {
		...
		/* upstream's value, for the models nobody has measured yet */
		connector->display_info.width_mm = 154;
		connector->display_info.height_mm = 86;
	}
	/*
	 * Drive pixel data on the negative clock edge -> atmel-hlcdc sets
	 * LCDC_LCDCFG0.CLKPOL=1, matching the MGS demo (CLKPOL(1)|CLKBYP(1)).
	 * Without this, CLKPOL=0 samples on the wrong edge at 50 MHz -> black.
	 */
...
static int ws_panel_bl_update_status(struct backlight_device *bl)
{
	...
	/*
	 * 0xAB is written DIRECTLY (not 0xff-brightness) — this panel variant is
	 * not inverted; the MGS demo writes 0xAB=55. 0xff-brightness gave 0 at
	 * max brightness => black image. 0xAA=1 latches it.
	 */
	ws_panel_i2c_write(ts, 0xab, backlight_get_brightness(bl));
	ws_panel_i2c_write(ts, 0xaa, 0x01);
...
ws_panel_create_backlight(struct ws_panel *ts)
{
	...
	const struct backlight_properties props = {
		.type = BACKLIGHT_RAW,
		.brightness = 55,	/* MGS demo value; 0xAB is direct, higher = brighter */
		.max_brightness = 255,
	};
...
static int ws_panel_probe(struct i2c_client *i2c)
{
	...
	/*
	 * On-panel MCU (I2C 0x45) power-on init — EXACTLY the MGS demo initSeq
	 * (curiosity_ws_4in_round ws_touch_display.c): C0,C2,AC,AD = 1, 1 ms
	 * apart. NO AES. Brightness (0xAB/0xAA) is deliberately NOT done here —
	 * the demo writes it only AFTER the DSI video is up (see ws_panel_enable).
	 */
